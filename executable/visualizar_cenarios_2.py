...
def _classify_impacts_by_coverage_2d(
    ...
    usando critério geométrico 2D: existe sensor com distância <= max_det.
    """
    ...
    dmin = d.min(axis=1) if d.size else np.full((impacts_xy.shape[0],), np.inf, dtype=float)
    return dmin <= float(max_det)
...
def plot_scene(
    *,
    ...
    out_of_coverage_mask: Optional[np.ndarray] = None,  # (M,) True => fora de cobertura
    coverage_mode: str = "2d_distance",  # "2d_distance" (padrão)

    show_inter_sensor_lines: bool = True,
    inter_sensor_lines_mode: str = "limit",  # "all" | "limit" | "mst"
    max_lines: int = 8,  # usado no modo "limit"
    ...
    """
    - Se out_of_coverage_mask vier None:
        - e highlight_out_of_coverage=True, calcula por distância 2D usando max_det.
    - Linhas entre boias:
        - all: desenha todas (pode poluir)
        - limit: desenha só as menores distâncias (max_lines)
        - mst: desenha MST (bem paper-friendly)
    """
    ...
    # impactos: coberto vs fora de cobertura
    if highlight_out_of_coverage:
        if out_of_coverage_mask is None:
            if coverage_mode == "2d_distance":
                covered_mask = _classify_impacts_by_coverage_2d(impacts_xy, sensors, max_det=max_det)
                out_of_coverage_mask = ~covered_mask
            else:
                raise ValueError(f"coverage_mode desconhecido: {coverage_mode}")
        ...
        if out_of_coverage_mask.any():
            ax.scatter(
                ...
                s=18,
                marker="x",
                label="Impact points (out of coverage)",
            )
    else:
...
def generate_all_figures(
    *,
    ...
    for gen in range(num_generations):
        ...
        # Observação:
        # Aqui eu NÃO tenho o mask real "quais impactos ficaram sem cobertura" vindo do report,
        # então o plot usa critério geométrico 2D com maximum_detection_distance.
        # Se você quiser 100% idêntico ao evaluator, eu adapto para usar um mask vindo do report
        # (basta você me dizer como o report representa isso / adicionar no report).
        #
        # Por enquanto: out_of_coverage_mask=None => calcula via distância 2D automaticamente.
        ...
        plot_scene(
            ...
            out_of_coverage_mask=None,  # auto
            coverage_mode="2d_distance",
...
if __name__ == "__main__":
    ...
    generate_all_figures(
        ...
        # sugestões boas pra paper:
        highlight_out_of_coverage=True,
        inter_sensor_lines_mode="mst",   # mst fica limpo e informativo
        max_lines=8,                     # só se você trocar para mode="limit"
